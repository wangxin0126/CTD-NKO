...
def generate_x_v2(X_hist, A_t, params):
    ...
    non_linear_transforms = [
        ...
        lambda x: x**0.5,
        lambda x: x,
        lambda x: 1 - np.cos(x * 3.14159 * 2.),  # 添加1以避免log(0)
        lambda x: x**2,
        lambda x: np.sin(x * 3.14159 * 2.),  # 开方
    ]
    ...
    # X_new = np.dot(params['theta_x'] * non_linear_A_t, np.mean(X_hist, axis=0))
    # X_new = np.dot(params['theta_x'] * non_linear_A_t, X_hist[-1])
    # X_new = np.dot(np.mean(X_hist, axis=0), params['theta_x'] * non_linear_A_t)
    X_new = np.dot(X_hist[-1], params['theta_x'] * non_linear_A_t)

    # 加入噪声
    X_new += np.random.normal(scale=params['noise_scale_x'], size=X_new.shape)
    # normalize X_new to N(0, 1)
...
def generate_y(X_hist, A_t, params):
    ...
    x6 = x[5]
    t = A_t
    # Y_new = np.cos((t-0.5) * 3.14159 * 2.) * (t**2 + (4.*np.max((x1, x6))**3)/(1. + 2.*x3**2)*np.sin(x4))
    # Y_new = (1 - np.cos(t * 3.14159 * 3.)) * x1 * (1. / (1. + x3**2)) + t**2 * x4 * (t * (2. / (1. + x6**2)) - 1)
...
def simulate_time_series(T, h, params, mode_x='v2', mode_a='v1'):
    """模拟一个时间序列数据，其中T是时间序列的长度，h是历史窗口的大小"""
    # 定义X的维度
